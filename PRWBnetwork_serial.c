...
//		count : 0... sum_column
		...
		//	i : 0...N-1
			Cm_pr  = 3.0;
			am_pr = - (0.32*(y[i*(N+7)+0]+46.9))/(exp(-(y[i*(N+7)+0]+46.9)/4.0) - 1.0);
			an_pr =  - (0.016*(y[i*(N+7)+0]+24.9))/(exp(-(y[i*(N+7)+0]+24.9)/5.0)-1.0);
			ah_pr = 0.128 * exp((-43-y[i*(N+7)+0])/18.0);
			as_pr = 1.6/(1.0 + exp(-(0.072*(y[i*(N+7)+1]-5.0))));
			bm_pr  =  (0.28*(y[i*(N+7)+0]+19.9))/(exp((y[i*(N+7)+0]+19.9)/5.0) - 1.0);
			bn_pr  = 0.25 * exp(-(y[i*(N+7)+0]+40.0)/40.0);
			bh_pr  = 4.0/(1.0 + exp(-(y[i*(N+7)+0]+20)/5.0));
			bs_pr  = (0.02*(y[i*(N+7)+1]+8.9))/(exp((y[i*(N+7)+1]+8.9)/5.0) - 1.0);
			...
			m_inf_pr = am_pr/(am_pr+bm_pr);

			gDR_pr = gmax[2]*y[i*(N+7)+3];
			gNa_pr = gmax[1]*m_inf_pr*m_inf_pr*y[i*(N+7)+4];
			gCa_pr = gmax[3]*y[i*(N+7)+5]*y[i*(N+7)+5];
			if (y[i*(N+7)+2]/250<1)
				gC_pr = gmax[5]*y[i*(N+7)+6]*y[i*(N+7)+2]/250.0;
			else
					gC_pr = gmax[5]*y[i*(N+7)+6];
			gAHP_pr = gmax[4]*y[i*(N+7)+7]; 
		...
		//	initialize iterators
			...
			am_wb = -0.1*(y[i*(N+7)+0]+35)/(exp(-0.1*(y[i*(N+7)+0]+35))-1);
			bm_wb  = 4*exp(-(y[i*(N+7)+0]+60)/18);
			ah_wb = 0.07*exp(-(y[i*(N+7)+0]+58)/20);
			bh_wb  = 1/(exp(-0.1*(y[i*(N+7)+0]+28))+1);
			an_wb = -0.01*(y[i*(N+7)+0]+34)/(exp(-0.1*(y[i*(N+7)+0]+34))-1);
			bn_wb  = 0.125*exp(-(y[i*(N+7)+0]+ 44)/80);

			IK_wb = gK_wb*(y[i*(N+7)+1]*y[i*(N+7)+1]*y[i*(N+7)+1]*y[i*(N+7)+1])*(y[i*(N+7)+0]-EK_wb);
			IL_wb = gL_wb*(y[i*(N+7)+0]-EL_wb);
			m_inf_wb = am_wb/(am_wb+bm_wb);
			INa_wb = gNa_wb*m_inf_wb*m_inf_wb*m_inf_wb*y[i*(N+7)+2]*(y[i*(N+7)+0]-ENa_wb);	
...
//		for each of the synaptic variables
		for (j = 0;j<N;j++){
			if (j!=i){	
				F_Pre = 1.0/(1.0+exp(-(y[mat[j]]-theta)/2.0));
